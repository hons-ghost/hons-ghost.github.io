...
export class Island implements IPhysicsObject {
    ...
    async Loader(scale: number, position: CANNON.Vec3) {
        return new Promise((resolve) => {
            this.loader.Load.load("assets/island/island.glb", (gltf) => {
                ...
                /*
        Gui.add(this.body.position, "x", -100, 100).step(1).listen()
        Gui.add(this.body.position, "y", -100, 100).step(1).listen()
        Gui.add(this.body.position, "z", -100, 100).step(1).listen()
        */
                resolve(gltf.scene)
            })
        })
